from src.modeling.launch_ec2_final_training_job import (
    build_run_instances_command,
    build_user_data_script,
)


def test_build_user_data_script_restores_features_runs_final_training_and_syncs_outputs():
    script = build_user_data_script(
        ...
        modeling_s3_prefix="s3://example-bucket/example-prefix/modeling-final",
    )
    ...
    assert (
        "-m src.modeling.train_anomaly_model --artifact-root artifacts/features --output-root artifacts/modeling --final-train"
        in script
    )
    assert "aws s3 sync /opt/rad/modeling s3://example-bucket/example-prefix/modeling-final" in script
    assert "git clone" not in script
    assert "python3 -m venv" not in script
    assert "pip install -r requirements.txt" not in script


def test_build_run_instances_command_tags_final_training_job():
    command = build_run_instances_command(
        ...
        tag_name="rad-final-train",
        user_data_script="#!/bin/bash\necho hi\n",
    )

    joined = " ".join(command)
    assert command[:4] == ["aws", "ec2", "run-instances", "--image-id"]
    assert "rad-final-train" in joined
